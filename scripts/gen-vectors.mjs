...
//   - ML-DSA: keygen(seed) is deterministic -> pk must match; verify(pk,msg,sig)==true
//   - ML-KEM: keygen(seed) deterministic -> (pk,sk); decapsulate(sk,ct)==ss
//   - SHAKE-256: shake256(msg, dkLen) must match
//
// Run: node scripts/gen-vectors.mjs   (writes ../vectors/*.json)
...
function mldsa(name, scheme) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const msg = msgN(i);
    const sig = scheme.sign(msg, secretKey);
...
function mlkem(name, scheme) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const { cipherText, sharedSecret } = scheme.encapsulate(publicKey);
...
function shake() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    cases.push({ message: hex(msg), out32: hex(shake256(msg, { dkLen: 32 })), out64: hex(shake256(msg, { dkLen: 64 })) });
  }
